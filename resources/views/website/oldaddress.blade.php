...
$editId=$userdetails->id;
  ...
  $setLocation=Location::where('id',session()->get('location'))->get();
  $allLocation=Location::where('cate_id',$selectCity)->get();
}
?>
...
@foreach($allLocation as $allLocationData)
<option <?php if($allLocationData->id==session()->get('location')){ echo"selected";} ?> value="{{$allLocationData->id}}">
...
<div class="input-group">
 <div class="input-group-prepend">
  ...
  <input type="text" name="time" readonly autocomplete="off" id="addressSetTime" class="form-control time tm" placeholder="Time" aria-label="time" value="{{$bokTime}}" aria-describedby="basic-addon1" required="">
</div>
<!---------- Date Time ------->               
