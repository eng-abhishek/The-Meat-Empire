...
<!--                   <a href="{{url('deal-of-day-add')}}" style="float: right"><button class="btn btn-info"><i class="fa fa-plus"></i>Add Deal of Day</button></a>  -->
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr>
                        ...
                        <th>Pack of Surprise</th>
                        <th>Status</th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    <tbody>
                   @foreach($discount as $key => $discountval)
                   <tr>
                   <td>{{$key + 1}}</td>
                   ...
                   <td>
                    <?php $specialoffer=''; ?>
                   @if($discountval->pack_of_surprise)
                   <?php $specialoffer=$discountval->pack_of_surprise;?>
                   @else 
                  <?php  $specialoffer='0';?>
                   @endif
                    {{$specialoffer}}</td> 
 ...
 <a href="{{url("discount-edit/$discountval->id")}}" class="btn btn-info"><i class="fa fa-edit"></i></a>
                       </td>
                     </tr>
                      @endforeach
                    </tbody>                   
...
function updateDealOfDayStatus(id){
 ...
 $.ajax({
 url:"{{url('update-discount-status')}}",
 method:'post',
